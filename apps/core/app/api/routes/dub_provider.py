...
async def list_dub_provider_voices(
    refresh: bool = Query(default=False, description="Bypass the cached provider voice list."),
):
    try:
        voice_list = await run_in_threadpool(dub_provider_client.list_voices, refresh)
